        ...
        if (mAnimationStartTime > 0) {
            when(mAnimationMode) {
                ...
                BaseAttrTextLayout.ANIMATION_CONTINUATION_ERASE_Y -> {
                    ...
                    if (!mIsCurrentView) {
                       if (mAnimationTop) {
                           canvas.clipRect(0f, heightFloat * mAnimationTimeFraction, width.toFloat(), heightFloat)
                       } else {
                           canvas.clipRect(0f, 0f, width.toFloat(), heightFloat - heightFloat * mAnimationTimeFraction)
                       }
                    } else {
                        // 剪切当前区域
