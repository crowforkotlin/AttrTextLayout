 ...
 * @formatter:on
 */
internal class AttrTextView_Copy internal constructor(context: Context) : View(context), IAttrText {
    ...
    companion object {
         ...
         * ● 高刷定时器单例
         *
         * ● 2024-02-18 14:59:28 周日 下午
         * @author crowforkotlin
         */
        private val mHighBrushTimer = Timer()
    }
    ...
    private val mHandlerThread = HandlerThread("HANDLER")
    ...
    override fun onDraw(canvas: Canvas) {
        ...
        when(mGravity) {
            ...
            GRAVITY_TOP_CENTER -> {
                drawTopText(canvas, text, textListSize,
                    onInitializaTextY = { tryLayoutHighBrushY(textAxisValue, it) },
                    onInitializaTextX = { tryLayoutHighBrushX(textAxisValue, (width shr 1) - it / 2f) }
                )
            }
            GRAVITY_TOP_END -> {
                drawTopText(canvas, text, textListSize,
                    onInitializaTextY = { tryLayoutHighBrushY(textAxisValue, it) },
                    onInitializaTextX = { tryLayoutHighBrushX(textAxisValue, width - it) }
                )
            }
    ...
    private fun tryLayoutHighBrushY(textAxisValue: Float, originY: Float) : Float {
        ...
        if (mTextAnimationMode == ANIMATION_MOVE_Y_HIGH_BRUSH_DRAW) {
            drawView(
                onCurrent = { y +=((if(mTextAnimationTopEnable) height.toFloat() else -height.toFloat())) + textAxisValue },
    ...
    private fun tryLayoutHighBrushX(textAxisValue: Float, originX: Float)  {
        if (mTextAnimationMode == ANIMATION_MOVE_X_HIGH_BRUSH_DRAW) {
            drawView(
                onCurrent = { mTextX = (if(mTextAnimationLeftEnable) width + originX else -(width - originX)) + textAxisValue },
                onNext = {
    ...
    internal fun launchHighBrushDrawAnimation(isX: Boolean, duration: Long = IAttrText.DRAW_VIEW_MIN_DURATION) {
        ...
        if (isX) {
            ...
            launchHighBrushSuspendAnimation(height, mTextAnimationTopEnable, duration)
        }
    }
    ...
    private fun invalidateHighBrushAnimation(duration: Long) {
        if (mHighBrushTopOrLeft) {
            ...
            mTextAxisValue ++
            if (mTextAxisValue < mHighBrushPixelCount) {
                if (duration == 0L) invalidate() else mHighBrushTimer.schedule(object : TimerTask() {
                    override fun run() {
                        if (mTextAxisValue < mHighBrushPixelCount) invalidate() else {
                            cancel()
                            mHighBrushJobRunning = false
                            mHighBrushSuccessListener?.run()
                        }
